...
from database.postservice import get_all_post_db, get_exact_post_db, delete_exact_post_db, post_new_photo_db, add_new_post_db, create_post_for_hashtag
from database.postservice import change_post_text_db
post_bp = Blueprint('post', __name__, url_prefix='/post')

@post_bp.route('/', methods = ['GET'])
def get_all_user_posts():
    all_posts = get_all_post_db()
...
def change_user_post( post_id: int):
     ...
     return {'status':1, 'message':'Upload'}

@post_bp.route('/<int:user_id>/<int:post_id>', methods=['DELETE'])
def delete_user_post(user_id: int, post_id: int): # user_id?
